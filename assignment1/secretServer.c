...
int main(int argc, char **argv)
{
    ...
    if (bind(listen_sock_fd, (struct sockaddr *)&srv_addr, sizeof(srv_addr)) < 0)
    {
    ...
    while (1)
    {
        ...
        read_line_from_fd(conn_sock_fd, globals.buffer); // this line overwrites the password
        printf("Password's Memory Address: %x\n", globals.password);
        printf("Buffers Memory Address: %x\n", globals.buffer);
        printf("Input Buffer is: %s\n", globals.buffer);           // This is the first set of A's
        printf("Overwritten password is: %s\n", globals.password); // this is the second set of A's
